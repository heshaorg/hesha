...
impl Claims {
    ...
    pub fn to_attestation(&self) -> hesha_types::HeshaResult<hesha_types::Attestation> {
        ...
        let hash_hex = self.phone_hash
            .strip_prefix("sha256:")
            .ok_or_else(|| hesha_types::HeshaError::InvalidAttestation("Invalid phone hash format".into()))?;
        ...
        Ok(hesha_types::Attestation {
            ...
            phone_hash: PhoneHash::from_bytes(
                hex::decode(hash_hex)
                    .map_err(|_| hesha_types::HeshaError::InvalidAttestation("Invalid phone hash hex".into()))?
                    .try_into()
                    .map_err(|_| hesha_types::HeshaError::InvalidAttestation("Invalid phone hash length".into()))?
            ),
            iss: self.iss.clone(),
            trust_domain: self.trust_domain.clone(),
            exp: Utc.timestamp_opt(self.exp, 0)
                .single()
                .ok_or_else(|| hesha_types::HeshaError::InvalidAttestation("Invalid expiry timestamp".into()))?,
            iat: Utc.timestamp_opt(self.iat, 0)
                .single()
                .ok_or_else(|| hesha_types::HeshaError::InvalidAttestation("Invalid issued timestamp".into()))?,
