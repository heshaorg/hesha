...
pub fn encode_jwt<T: Serialize>(claims: &T, private_key: &PrivateKey) -> HeshaResult<String> {
    // Create header
    ...
    // Encode claims
    let claims_json =
        serde_json::to_string(claims).map_err(|e| HeshaError::SerializationError(e.to_string()))?;
...
pub fn decode_jwt<T: for<'de> Deserialize<'de>>(
    ...
    if header.alg != "EdDSA" {
        return Err(HeshaError::InvalidAttestation(format!(
...
pub fn decode_jwt_unverified<T: for<'de> Deserialize<'de>>(jwt: &str) -> HeshaResult<T> {
    // Split JWT
    let parts: Vec<&str> = jwt.split('.').collect();
    if parts.len() != 3 {
        return Err(HeshaError::InvalidAttestation(
