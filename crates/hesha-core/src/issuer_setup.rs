...
use hesha_types::{HeshaError, HeshaResult, IssuerConfig, IssuerIdentity, KeyPair};
use std::fs;
use std::path::Path;
...
impl IssuerSetupBuilder {
    ...
    pub fn build(self) -> HeshaResult<IssuerSetup> {
        let name = self
            .name
            .ok_or_else(|| HeshaError::ConfigError("Issuer name is required".to_string()))?;
        let trust_domain = self
            .trust_domain
            .ok_or_else(|| HeshaError::ConfigError("Trust domain is required".to_string()))?;
        let contact_email = self
            .contact_email
            .ok_or_else(|| HeshaError::ConfigError("Contact email is required".to_string()))?;

        // Validate trust domain format
        if !is_valid_domain(&trust_domain) {
            return Err(HeshaError::ConfigError(
                "Invalid trust domain format".to_string(),
            ));
        }
        ...
        // Generate keypair
        let keypair = generate_keypair().map_err(|e| HeshaError::CryptoError(e.to_string()))?;
...
impl IssuerSetup {
    ...
    pub fn save(&self, base_dir: impl AsRef<Path>) -> HeshaResult<()> {
        ...
        fs::create_dir_all(&config_dir).map_err(|e| {
            HeshaError::ConfigError(format!("Failed to create config directory: {}", e))
        })?;
        fs::create_dir_all(&keys_dir).map_err(|e| {
            HeshaError::ConfigError(format!("Failed to create keys directory: {}", e))
        })?;
        ...
        #[cfg(unix)]
        {
            ...
            fs::set_permissions(&private_key_path, permissions).map_err(|e| {
                HeshaError::ConfigError(format!("Failed to set key permissions: {}", e))
            })?;
        }
        ...
        let pubkey_json = serde_json::to_string_pretty(&pubkey_endpoint).map_err(|e| {
            HeshaError::ConfigError(format!("Failed to serialize pubkey endpoint: {}", e))
        })?;
        fs::write(&pubkey_path, pubkey_json).map_err(|e| {
            HeshaError::ConfigError(format!("Failed to write pubkey endpoint: {}", e))
        })?;
